...
export default function ExerciseFeedback({ state, dispatch }) {
  ...
  const handleFeedback = (feedbackId) => {
    // Update the last exercise's feedback
    const updated = [...session.completedExercises];
    updated[updated.length - 1] = { ...updated[updated.length - 1], feedback: feedbackId };
      ...
      <div className="max-w-md w-full animate-fade-in-up">
        <div className="bg-white rounded-3xl border border-[var(--color-border)] p-8 text-center shadow-sm">
          ...
          <p className="text-xs text-[var(--color-text-muted)]">
            {isLast ? 'Last exercise — great session!' : `${exercises.length - session.exerciseIndex - 1} exercise${exercises.length - session.exerciseIndex - 1 !== 1 ? 's' : ''} remaining`}
          </p>
        </div>
      </div>
    </div>
